// Runnable contract test for "Select N in tree" on the newest run
// (selectableScannedKeys + the selectDeployed handler, panelProvider.ts).
// No framework.   1) npm run compile   2) node scripts/check-select-deployed.cjs
//
// The button's keys come from the webview — the rows the run's list shows that
// the workspace scan has local source for — and a run outlives the scan it was
// drawn against, so on the way back they are untrusted and possibly stale.
// Contract under test: only a key that names a CURRENTLY scanned item with local
// source may reach the tree — a forged, renamed, deleted or org-only key is
// dropped and counted, so the panel can say what it lost instead of quietly
// selecting fewer components than the button promised — and a run's own rows,
// offered by that rule, never report a phantom loss. Same security rule as the
// suggestion flow: run text can never mint a --metadata key.
//
// selectableScannedKeys is a pure prototype method that never touches `this`,
// so it is called directly rather than standing up a provider + webview.
...
const { deployRunFromResult } = require(path.join(__dirname, '..', 'out', 'runRecords.js'));
...
// ------------------------------------------ what the webview offers to select
// The run's list decides nothing on its own: the button carries the rows it
// shows that the CURRENT scan has local source for (panel.js renderRunActs,
// runView.actionsFor). Built here from a real run the same way.
const runOf = (items, skipped = []) => deployRunFromResult({ status: 'Succeeded', success: true }, {
  id: 'runsel01', op: 'deploy', org: 'acme-dev-user', orgLabel: 'acme-dev', orgKind: 'sandbox',
  startedAt: 1, finishedAt: 2, target: 'selection', items, skipped: { orgOnly: skipped, unread: [] }
});
const offered = (run, items) => {
  const local = new Set(items.filter(i => !!i.filePath).map(i => `${i.type}:${i.name}`));
  return [...new Set(run.rows.map(r => r.k).filter(k => local.has(k)))];
};

check('a run offers exactly its rows with local source — never a skipped, org-only one', () => {
  const run = runOf(ITEMS.filter(i => !!i.filePath), [{ type: 'PermissionSet', name: 'AcmeOrgOnly' }]);
  assert.deepStrictEqual(offered(run, ITEMS), ['ApexClass:AcmeOrderService', 'ApexClass:AcmeOrderServiceTest', 'CustomObject:Widget__c', 'CustomField:Widget__c.Size__c']);
});

check('an org-reported row the workspace does not have is never offered', () => {
  const report = deployRunFromResult({ status: 'Succeeded', success: true, files: [
    { type: 'ApexClass', fullName: 'AcmeOrgOnlyService', state: 'Changed' },
    { type: 'ApexClass', fullName: 'AcmeOrderService', state: 'Changed' }
  ] }, { id: 'runsel02', op: 'deploy', org: 'acme-dev-user', orgLabel: 'acme-dev', orgKind: 'sandbox', startedAt: 1, finishedAt: 2, target: 'report' });
  assert.deepStrictEqual(offered(report, ITEMS), ['ApexClass:AcmeOrderService']);
});
...
// ----------------------------------------------------------------- round trip
check('what a run offers → scan gate: it re-selects exactly its own local rows', () => {
  const deployed = ITEMS.filter(i => !!i.filePath).map(i => `${i.type}:${i.name}`);
  const run = runOf(ITEMS.filter(i => !!i.filePath), [{ type: 'PermissionSet', name: 'AcmeOrgOnly' }]);
  assert.deepStrictEqual(resolve(offered(run, ITEMS), ITEMS), { keys: deployed, dropped: 0 });
});

check('a run offered against the current scan never reports a phantom loss', () => {
  // The `dropped` count is shown as "N no longer in the workspace", so it must
  // only ever count REAL losses: offering only locally-backed rows is what
  // guarantees that for an unchanged workspace.
  const run = runOf(ITEMS.filter(i => !!i.filePath), [{ type: 'PermissionSet', name: 'AcmeOrgOnly' }]);
  assert.strictEqual(resolve(offered(run, ITEMS), ITEMS).dropped, 0, 'the org-only member must never have been offered');
});
...
// already ticked — and a run is kept history, so that is the normal case,
// not the corner one.
...
const asyncChecks = [
  ['clicking Select REPLACES the selection with that run', async () => {
  ...
  ['a stale run says how many it lost, and selects the rest', async () => {
  ...
  ['a run whose components are ALL gone never touches the selection', async () => {
